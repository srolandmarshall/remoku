...
var remokuVarNames =
  "rokuAddress manualRokus namedRokus scannedRokus myNetwork rokuCount macros showFavs fav1 fav2 fav3 favsList bgColor fgColor".split(
    " "
  );
...
function loadConfigFromText() {
  if (
    window.confirm(
      "Overwrite your current settings with the content of the import/export field?"
    )
  ) {
    try {
      var importedConfigs = {};
...
function wipeConfig() {
  if (
    window.confirm(
      "Are you sure you want to wipe your settings and reload? You can cancel and export them first."
    )
  ) {
    for (var i = 0; i < remokuVarNames.length; i++) {
...
function dbg(log) {
  if (typeof JSON !== "undefined" && typeof log !== "string")
...
function ver(channel, build) {
  var slicescript =
    "window.external.addToFavoritesBar('http://remoku.tv/', 'Remoku', 'slice');";
  var webslice =
    '<a onclick="' + slicescript + '">Remoku</a> ' + channel + "<br>" + build;
  ver = $("ver");
...
function preventMove(event) {
  if (
    ...
    remoteScreen.getAttribute("class") == "visible"
  )
...
function updateSelect() {
  ...
  for (i = 0; i < rokus.length; i++) {
    if (rokus[i] !== null) {
      ...
      var thisRokuName =
        namedRokus && namedRokus[rokus[i]] ? namedRokus[rokus[i]] : rokus[i];
      rokuSelect.options[i] = new Option(
        thisRokuName,
        rokus[i],
        rokuSelected,
        rokuSelected
...
function buildManualRokusMenu() {
  ...
  for (i = 0; i < manualRokus.length; i++) {
    ...
    //alert(manualRokus[i]);
    manualSelect.options[i] = new Option(
      manualRokus[i],
      manualRokus[i],
      rokuSelected
    );
...
function imageError() {
  ...
  if (ipCount >= 254) {
    ...
    if (scannedRokus.length < 1)
      scanResults.innerHTML = "No Rokus Found. Check your network settings.";
    if (scannedRokus.length < 1)
      dbg("No Rokus Found. Check your network settings.");
  } else if (ipPos < 255) {
    ipPos++;
    images[ipPos].src = URLS[ipPos];
    timeouts = setTimeout("cancelImage(" + ipPos + ");", 500);
...
function cancelImage(i) {
  ...
  if (scanning) {
    ...
    scanResults.innerHTML =
      "Done scanning, found " +
      scannedRokus.length +
      " Rokus.  If not all of your Rokus have been discovered, please scan again, then look to <a class='bgcolor' href='http://help.remoku.tv/'>help.remoku.tv,</a> if you need more help.";
  }
  if (ipCount >= 254) {
    scanButton.innerHTML = "Scan";
    scanning = false;
...
function rokuFrameLoad() {
  if (actions.length > 0) {
    ...
    rokupost.setAttribute(
      "action",
      "http://" + rokuAddress + ":8060/" + action.cmd + "/" + action.param
    );
    rokupost.submit();
  }
}
  ...
  if (cmds.length > 0) {
    ...
    switch (cmdAction) {
      case "wait":
        ...
        }, hmsToSecondsOnly(cmdParam) * 1000);
        break;
      case "text":
...
function rokuMacroText(cmdParam) {
  ...
  if (text) {
    ...
    if (letter == "/") {
      ...
      letter = "%2f";
      //      dbg("  " + letter);
      rokutext.setAttribute(
        "action",
        "http://" + rokuAddress + ":8060/" + "keypress" + "/" + "LIT_" + letter
      );
    } else if (letter == "@") {
      //      dbg(letter);
      letter = "%40";
...
function launchShoutCast() {
  ...
  params =
    "name=" +
    escape(shoutCastNameInput.value) +
    "&url=" +
    escape(shoutCastUrlInput.value).split("/").join("%2F");
  rokupost.setAttribute(
    "action",
    "http://" + rokuAddress + ":8060/launch/2115?" + params
  );
  rokupost.submit();
  return false;
}
...
function launchNowhereCast() {
  ...
  rokupost.setAttribute(
    "action",
    "http://" + rokuAddress + ":8060/launch/22358?" + params
  );
  rokupost.submit();
  return false;
}
...
function launchRemokuWithParams() {
  ...
  rokupost.setAttribute(
    "action",
    "http://" + rokuAddress + ":8060/launch/dev?" + params
  );
  rokupost.submit();
  return false;
}
  ...
  switch (evt.keyCode) {
    ...
    case 13:
      if ($("textentry").value === "") {
        rokutext.setAttribute(
          "action",
          "http://" + rokuAddress + ":8060/" + "keypress" + "/" + "Enter"
        );
        rokutext.submit();
      }
      break;
...
function delayLoadIcons() {
  ...
  if (appid)
    $("app" + appid).src =
      "http://" + rokuAddress + ":8060/query/icon/" + appid;
}
...
function loadNextFav() {
  var favsList = getConfig("favsList")
    ? JSON.parse(getConfig("favsList"))
    : ["12", "28", "2016"];
  for (var i = 0; i < favsList.length - 1; i++) {
    if (this.id === "fav-" + favsList[i]) {
      document.getElementById("fav-" + favsList[i + 1]).src =
        "http://" + rokuAddress + ":8060/query/icon/" + favsList[i + 1];
    }
  }
}
...
function setupFavs() {
  ...
  if (favsList.length > 0 && rokuAddress)
    document.getElementById("fav-" + favsList[0]).src =
      "http://" + rokuAddress + ":8060/query/icon/" + favsList[0];
}
...
function saveFavs() {
  ...
  for (i = 0; i < favs.length; i++) {
    if (favs[i].getElementsByTagName("input")[0].value !== "")
      favsList.push(favs[i].getElementsByTagName("input")[0].value);
  }
  setConfig("favsList", JSON.stringify(favsList));
    ...
    function (e) {
      if (window.applicationCache) {
        ...
        window.applicationCache.addEventListener(
          "downloading",
          function () {
            dbg("appCache: Downloading Update ");
          },
          false
        );
...
function ffInstall(aEvent) {
  var params = {
    Remoku: {
      URL: "https://addons.mozilla.org/firefox/downloads/latest/362455/addon-362455-latest.xpi",
      IconURL:
        "https://addons.cdn.mozilla.net/img/uploads/addon_icons/362/362455-64.png",
    },
  };
...
window.onload = function () {
  ...
  scannedRokus = getConfig("scannedRokus")
    ? getConfig("scannedRokus").split(",")
    : [];
  ...
  manualRokus = getConfig("manualRokus")
    ? getConfig("manualRokus").split(",")
    : [];
  ...
  channelsLink.innerHTML =
    'Refer to your <a class="bgcolor" href="http://' +
    rokuAddress +
    ':8060/query/apps" target="_blank">installed channels</a> for app ids.';
  var channelsLink2 = $("channelslink2");
  channelsLink2.innerHTML =
    'Refer to your <a class="bgcolor" href="http://' +
    rokuAddress +
    ':8060/query/apps" target="_blank">installed channels</a> for app ids.';
  ...
  try {
    namedRokus = JSON.parse(getConfig("namedRokus"))
      ? JSON.parse(getConfig("namedRokus"))
      : {};
  ...
  navApps.onclick = function () {
    textModeOff();
    activateButton(this.id);
  };
  navConfig.onclick = function () {
    textModeOff();
    activateButton(this.id);
  };
  navGoodies.onclick = function () {
    textModeOff();
    activateButton(this.id);
  };
  ...
  if (typeof chrome !== "undefined" && chrome.webstore.install) {
    $("chromeInstall").innerHTML =
      '<button onclick="chrome.webstore.install()" id="install-button">Install Chrome Extension</button>';
  }
  if (navigator.userAgent.toLowerCase().indexOf("firefox") > -1) {
    $("chromeInstall").innerHTML =
      '<button onclick="ffInstall(event)" id="install-button">Install Firefox Extension</button>';
  }

  if (window.screen.height == 568) {
    // iPhone with 4 inch screen workaround for letterboxed home screen web app
    document.querySelector("meta[name=viewport]").content =
      "width=320.1, initial-scale=1.0, maximum-scale=1.0, user-scalable=no";
  }
  setupFavs();
};
